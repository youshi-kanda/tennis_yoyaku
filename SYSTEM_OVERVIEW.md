...
**理由**:
1. **無料プランでは1コート31日間監視が不可能**（サブリクエスト50制限）
2. CPU時間30秒/実行に緩和（無料: 10ms）
3. サブリクエスト1000件/実行（無料: 50件）
4. KV書き込み無制限でテスト・開発が自由
5. 複数コート監視が可能
...
**実際の制限（実測）**:
```
❌ 1コート31日間 = 124サブリクエスト/実行 → 50超過で失敗
❌ CPU時間10ms → バッチ処理でも超過
✅ 1コート7日間のみ可能（28サブリクエスト）
```

**誤算の原因**:
- サブリクエスト制限は「/日」ではなく「/実行」
- 1日のチェック = セッション + ログイン + 取得 = 4リクエスト
- 31日 × 4 = 124リクエスト → 50制限の2.5倍
