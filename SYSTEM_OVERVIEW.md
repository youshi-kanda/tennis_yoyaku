...
- **リクエスト**: 100,000/日
- **KV書き込み**: 1,000/日
- **サブリクエスト**: 50/リクエスト **⚠️ /実行（Cron実行ごと）**
- **CPU時間**: 10ms/実行 **⚠️ 実行ごと**
- **Cron**: 最短1分間隔
- **制約**: 秒単位監視不可

**⚠️ サブリクエスト数は実装次第で大きく変わります**:

**現在の実装（2025-11-26時点）**:
- **1コート31日監視**: 124サブリクエスト/実行（❌ 50超過）
  - 1日のチェック = セッション確認 + ログイン + 取得 = 約4リクエスト
  - 31日 × 4リクエスト/日 = 124サブリクエスト
- **CPU時間**: 10ms超過で強制終了

**実装最適化後の可能性**:
- 1検索で複数日付を取得: 1リクエスト/日 → 30日で30サブリクエスト（✅ 可能）
- セッション再利用の徹底: さらに削減可能
...
- **リクエスト**: 10M/月
- **KV書き込み**: 無制限
- **サブリクエスト**: 1,000/リクエスト **✅ /実行**
- **CPU時間**: 30秒/実行 **✅ 大幅緩和**
- **Cron**: 最短1分間隔
- **制約**: 秒単位監視にはDurable Objects必要

**実現可能な監視**:
- 複数コート × 31日間: 実装次第で対応可能
- CPU時間30秒: バッチ処理が余裕を持って実行可能
- 無料プランのサブリクエスト制限を気にせず開発・テスト可能

**推奨構成**:
- 実装最適化前: 1-5コート程度
- 実装最適化後: 10コート以上も可能
- まずは無料プランで実測し、必要に応じてアップグレード検討**✅ 大幅緩和**
- **Cron**: 最短1分間隔
- **制約**: 秒単位監視にはDurable Objects必要

**実現可能な監視**:
```
複数コート × 31日間: 100-500サブリクエスト/実行（✅ 余裕）
10コート × 31日間: 1,240サブリクエスト/実行（❌ 超過）
→ バッチ処理で分割すれば対応可能
```
...
globalThis.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
  subrequestCount++;
  console.log(`[Subrequest ${subrequestCount}/50] ${input}`);
  return originalFetch(input, init);
};

// Cron実行後
console.log(`Total: ${subrequestCount}/50`);
if (subrequestCount > 50) {
  console.error(`❌ 無料プラン制限超過: ${subrequestCount - 50}リクエスト over`);
}
```

**ステップ2: 実測結果に基づく判断**
- 50以下: ✅ 無料プランで運用可能
- 50超過: 実装最適化 or Workers Paid ($5/月) を検討
...
**現在の実装（未最適化）**:
- 1コート31日 = 124サブリクエスト（❌ 超過）

**最適化後の可能性**:
- 1コート31日 = 30-40サブリクエスト（✅ 可能）
- 複数コート対応も可能

### 📋 アップグレード手順（必要な場合）

**実測後に50超過が確認され、実装最適化でも解決できない場合のみ**:

1. Cloudflareダッシュボード > Workers & Pages
2. "Upgrade to Paid" をクリック
3. $5/月プランを選択
4. 既存のWorkerが自動的に有料プラン適用

**即座に制限緩和**:
- サブリクエスト: 50 → 1000（20倍）
- CPU時間: 10ms → 30秒（3000倍）

**注意**: まずは無料プランで実測し、本当に必要か確認してからアップグレードすること
