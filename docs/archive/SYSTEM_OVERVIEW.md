...
- **集中監視** (品川区「取」マーク検知時):
  ...
  - 10分ごとに継続監視、リソース使用量95%削減
  - 終了条件: ①予約成功 ②取マーク消失 ③予約時刻到達
- **並列処理**: 集中監視中も他のターゲットは独立して動作
- **ステータス表示**:
  - 監視中 (monitoring)
  - キャンセル検知 (detected)
  - 予約成功 (reserved)
...
interface MonitoringTarget {
  ...
  nextIntensiveCheckTime?: number;    // 次の集中監視時刻（10分単位のタイムスタンプ）
...
- **Version ID**: `2ba19ef2-99be-4226-b908-637611248554`
- **Compatibility Date**: 2024-01-01
- **Last Deploy**: 2025-11-30
- **Security**: AES-256-GCM パスワード暗号化実装済み
- **新機能**: 品川区「取」マーク集中監視（10分単位・1秒間隔・リソース95%削減）

### Cron Schedule
- **実行間隔**: 1分毎 (`*/1 * * * *`)
- **実行内容**:
  1. アクティブな監視ターゲット取得（全ユーザー分）
  2. 並列処理で全ターゲットを同時チェック
  3. 各サイトにログイン（セッション期限切れ時のみ）
  4. スクレイピング実行（通常監視・集中監視）
  5. キャンセル検知（×→○→取）
  6. 集中監視実行（「取」検知時）
     - 10分単位の前後15秒間を1秒間隔でチェック
     - 10分ごとに継続、終了条件まで
  7. 自動予約実行
  8. プッシュ通知送信
...
## Cloudflare Workers プラン

### ✅ 現在のプラン: Workers Paid ($5/月) - 契約済み
**制限値**:
- **リクエスト**: 10,000,000/月（実質無制限）
- **KV書き込み**: 無制限
- **サブリクエスト**: 1,000/実行
- **CPU時間**: 30秒/実行
- **Cron**: 最短1分間隔
...
**運用可能範囲**:
```
1コート × 31日監視: 93サブリクエスト/実行
  → 使用率: 9.3% (✅ 十分な余裕)

10コート × 31日監視: 930サブリクエスト/実行
  → 使用率: 93% (✅ 実行可能)
...
### 参考）Durable Objects（秒単位監視が必要な場合）
- **用途**: ステートフル処理、秒単位ループ
- **コスト**: Workers Paid + 従量課金
- **推定月額**: $7-15/月
- **現状**: 未実装（Cron 1分間隔で対応中）
